...
class FilterBuilder(object):   # noqa:WPS214
    ...
    def build(self, filter_set: dict):
        """
        Create filters.

        Example: filter_set = {or: {'username__eq': '123'} }
        If conjunction operator (`or`, `and`) in keys than
        build recursively separate filter for it,
        join all filters together.

        Args:
            filter_set (dict): dict-like filter set to create sql-like filters.

        Raises:
            OperatorDoesNotExist: can't split operator and field name

        """
        for filter_key, filter_val in filter_set.items():
            if filter_key == 'or' and isinstance(filter_val, dict):
                self._build_or(filter_set=filter_val)
            elif filter_key == 'and' and isinstance(filter_val, dict):
                self._build_and(filter_set=filter_val)
            else:
                try:
                    field_name, operator = filter_key.split('__')
                except Exception as exception:
                    logger.exception(
                        'Filter creating: building was failed: {0}'.format(
                            exception,
                        ),
                    )
                    raise db_exc.OperatorDoesNotExist
                self.resolve_operator(operator=operator)(
                    field_name,
                    filter_val,
                )
    ...
    def _build_or(self, filter_set: dict):
    ...
    def _build_and(self, filter_set: dict):
